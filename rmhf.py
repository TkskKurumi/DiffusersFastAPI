...
def get_prompt(prompt_file):
    ...
    def _(k):
        if (k in j):
            ...
            return sep.join(random.sample(sample_from, n))
    prompt = _("prompt")
    neg = _("negative")
    return prompt, neg
...
def main():
    ...
    parser.add_argument("--normalizer", type=str, choices=["softmax", "clip_std", "clip_overflow"], default="softmax")
    ...
    for step in range(steps):
        ...
        repro = model.txt2img(prompt, steps=args.infer_steps, width=512, height=512, neg_prompt=neg)
        image0 = repro.result
        ...
        image0.save("./0.png")
        image1.save("./1.png")
        which = input_yn("./0.png <-> ./1.png which is better? (0/1)", "01")

        for idx, i in enumerate(interps):
            newvec = i.vec + which*direction[idx]*step_rate
            i.vec = newvec
